# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  catppuccin-fcitx5 = {
    pname = "catppuccin-fcitx5";
    version = "ce244cfdf43a648d984719fdfd1d60aab09f5c97";
    src = fetchgit {
      url = "https://github.com/catppuccin/fcitx5/";
      rev = "ce244cfdf43a648d984719fdfd1d60aab09f5c97";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-uFaCbyrEjv4oiKUzLVFzw+UY54/h7wh2cntqeyYwGps=";
    };
    date = "2022-10-05";
  };
  firefox-gnome-theme = {
    pname = "firefox-gnome-theme";
    version = "1efffcaa78904816f70dd493627412d299b23a52";
    src = fetchgit {
      url = "https://github.com/rafaelmardojai/firefox-gnome-theme";
      rev = "1efffcaa78904816f70dd493627412d299b23a52";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-JouEEHCgFk9WMAi1VFqNP+Ow5I5NFD+B2+3E6BG1Y3c=";
    };
    date = "2024-05-16";
  };
  san-francisco-pro = {
    pname = "san-francisco-pro";
    version = "8bfea09aa6f1139479f80358b2e1e5c6dc991a58";
    src = fetchgit {
      url = "https://github.com/sahibjotsaggu/San-Francisco-Pro-Fonts";
      rev = "8bfea09aa6f1139479f80358b2e1e5c6dc991a58";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-mAXExj8n8gFHq19HfGy4UOJYKVGPYgarGd/04kUIqX4=";
    };
    date = "2021-06-22";
  };
  vesktop = {
    pname = "vesktop";
    version = "463c42330e753002c3a6935c57bbdf4d5498e516";
    src = fetchgit {
      url = "https://github.com/Vencord/Vesktop/";
      rev = "463c42330e753002c3a6935c57bbdf4d5498e516";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-oHn+MEBnsh7vJ/v4hj63SstiOfxlEIlBWy2QdbvFFPA=";
    };
    date = "2024-05-17";
  };
  waybar = {
    pname = "waybar";
    version = "8ad7c75d79f76c27a9c6b87460b79d1787c8855b";
    src = fetchgit {
      url = "https://github.com/alexays/waybar";
      rev = "8ad7c75d79f76c27a9c6b87460b79d1787c8855b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-ND9F+BJeyKaGZFu94SWIj6HtyZTzWf5qVyp3cI3a7Is=";
    };
    date = "2024-05-18";
  };
  wezterm = {
    pname = "wezterm";
    version = "da2d0ad267f32fb7ac341872a4c74004a68bbfcf";
    src = fetchgit {
      url = "https://github.com/wez/wezterm";
      rev = "da2d0ad267f32fb7ac341872a4c74004a68bbfcf";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-sP30a/2pfI4VWOI4FE93KavgFGCLHr2R5wCpZ4IaHfE=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./wezterm-da2d0ad267f32fb7ac341872a4c74004a68bbfcf/Cargo.lock;
      outputHashes = {
        "xcb-imdkit-0.3.0" = "sha256-77KaJO+QJWy3tJ9AF1TXKaQHpoVOfGIRqteyqpQaSWo=";
        "sqlite-cache-0.1.3" = "sha256-sBAC8MsQZgH+dcWpoxzq9iw5078vwzCijgyQnMOWIkk=";
      };
    };
    date = "2024-05-20";
  };
  whitesur-gtk-theme = {
    pname = "whitesur-gtk-theme";
    version = "cc39a8d37ae2b5ee43243cab7827c300af3ae902";
    src = fetchgit {
      url = "https://github.com/vinceliuice/WhiteSur-gtk-theme";
      rev = "cc39a8d37ae2b5ee43243cab7827c300af3ae902";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-NmwgYxxkd4Lc6oMyoK1D56kZXFiFWvX73FEMyeSB9DQ=";
    };
    date = "2024-05-20";
  };
}
